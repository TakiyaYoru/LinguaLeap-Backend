...
UserSchema.virtual('isPremium').get(function() {
  try {
    if (!this.subscriptionType || !this.subscriptionExpiry) return false;
    return this.subscriptionType === 'premium' && 
           new Date(this.subscriptionExpiry) > new Date();
  } catch (error) {
    console.warn('⚠️ Error calculating isPremium:', error.message);
    return false;
  }
});
...
UserSchema.virtual('heartsRefillTime').get(function() {
  try {
    if (!this.hearts || this.hearts >= 5) return null;
    
    // Hearts refill every 30 minutes
    const lastHeartLoss = this.updatedAt || new Date();
    const refillTime = new Date(lastHeartLoss.getTime() + 30 * 60 * 1000);
    return refillTime;
  } catch (error) {
    console.warn('⚠️ Error calculating heartsRefillTime:', error.message);
    return null;
  }
});
